...
describe('pr:decline', () => {
  ...
  let declinePullRequestStub: sinon.SinonStub
  ...
  beforeEach(async () => {
    ...
    declinePullRequestStub = sinon.stub().resolves(mockResult)
    clearClientsStub = sinon.stub()
    formatAsToonStub = sinon.stub().returns('toon-output')

    const imported = await esmock('../../../src/commands/pr/decline.js', {
  ...
  it('calls declinePullRequest with correct args and outputs JSON', async () => {
    const cmd = new PrDecline(['42', 'my-repo', 'my-ws'], {root: process.cwd(), runHook: sinon.stub().resolves({successes: [], failures: []})} as any)
  ...
  it('outputs TOON format when --toon flag is used', async () => {
    const cmd = new PrDecline(['42', 'my-repo', 'my-ws', '--toon'], {root: process.cwd(), runHook: sinon.stub().resolves({successes: [], failures: []})} as any)
